 ...
 */
export async function runQueryParserAgent({
  query,
  targetId,
  franchiseId,
  mode = "fast-track",
  ...
  reasoning.push(
    `[NLP Scanner] Comprehending user query: "${rawQuery || targetId || "default"}"`,
  );
  ...
  if (rawQuery) {
    ...
    if (
      qLower.includes("star wars") ||
      qLower.includes("starwars") ||
      qLower.includes("jedi") ||
      qLower.includes("sith") ||
      qLower.includes("skywalker") ||
      qLower.includes("clone wars")
    ) {
      explicitFranchise = "starwars";
      detectedFranchiseKey = "starwars";
      reasoning.push(
        `[Franchise Locked] Star Wars universe detected from keywords.`,
      );
    } else if (
      qLower.includes("naruto") ||
      qLower.includes("shippuden") ||
      qLower.includes("shinobi") ||
      qLower.includes("leaf village")
    ) {
      explicitFranchise = "anime_naruto";
      detectedFranchiseKey = "anime_naruto";
      reasoning.push(
        `[Franchise Locked] Naruto universe detected from keywords.`,
      );
    } else if (
      qLower.includes("dragon ball") ||
      qLower.includes("dbz") ||
      qLower.includes("super saiyan") ||
      qLower.includes("dragonball")
    ) {
      explicitFranchise = "dragonball";
      detectedFranchiseKey = "dragonball";
      reasoning.push(
        `[Franchise Locked] Dragon Ball universe detected from keywords.`,
      );
    } else if (
      qLower.includes("marvel") ||
      qLower.includes("mcu") ||
      qLower.includes("avengers") ||
      qLower.includes("sacred timeline")
    ) {
      explicitFranchise = "mcu";
      detectedFranchiseKey = "mcu";
      reasoning.push(
        `[Franchise Locked] Marvel Cinematic Universe detected from keywords.`,
      );
    }
    ...
    if (
      ...
      reasoning.push(
        `[Intent Detection] User requested FORWARD progression ("What to watch AFTER...")`,
      );
    } else if (
      qLower.includes("before") ||
      qLower.includes("prepare") ||
      qLower.includes("prerequisite") ||
      qLower.includes("lead up")
    ) {
      detectedDirection = "before";
      reasoning.push(
        `[Intent Detection] User requested BACKWARD prerequisite path ("What to watch BEFORE...")`,
      );
    }

    // 3. Detect Strategy Mode
    if (
      qLower.includes("all releases") ||
      qLower.includes("complete catalog") ||
      qLower.includes("every movie") ||
      qLower.includes("entire universe") ||
      qLower.includes("all timeline")
    ) {
      selectedMode = "all";
      reasoning.push(`[Mode Resolved] Complete Universe Catalog.`);
    } else if (
      qLower.includes("fast") ||
      qLower.includes("quick") ||
      qLower.includes("essential") ||
      qLower.includes("speedrun") ||
      ...
      qLower.includes("no filler") ||
      qLower.includes("zero filler")
    ) {
    ...
    if (hoursMatch && hoursMatch[1]) {
      ...
      reasoning.push(
        `[Constraint Extraction] Target watch budget: ${detectedHours} hours.`,
      );
    }
    ...
    Object.entries(FRANCHISES).forEach(([fKey, fData]) => {
      ...
      fData.nodes.forEach((node) => {
        ...
        contentTokens.forEach((token) => {
          ...
          if (
            ["i", "ii", "iii", "iv", "v", "vi"].includes(token) &&
            nodeTitleLower.includes(token)
          ) {
            score += 50;
          }
    ...
    if (bestMatch && bestScore > 20) {
      ...
      if (detectedDirection === "after") {
        ...
        reasoning.push(
          `[Anchor Resolved] Starting AFTER node: "${bestMatch.title}" (${bestMatch.year}) [ID: ${bestMatch.id}]`,
        );
      } else {
        detectedTargetId = bestMatch.id;
        reasoning.push(
          `[Target Resolved] Target node: "${bestMatch.title}" (${bestMatch.year}) [ID: ${bestMatch.id}]`,
        );
      }
    }
  }
  ...
  const anchorNode = detectedAnchorId
    ? franchise.nodes.find((n) => n.id === detectedAnchorId)
    : null;
  const targetNode = detectedTargetId
    ? franchise.nodes.find((n) => n.id === detectedTargetId)
    : null;
